...
/*
 * Aurora — animated WebGL gradient background.
 * Adapted from reactbits.dev (MIT). Renders a flowing aurora band whose hue
 * ramps across `colorStops`. Falls back to the static `.aurora-fallback`
 * gradient when prefers-reduced-motion is set or WebGL is unavailable.
 */
...
export default function Aurora({
  colorStops = ["#6366f1", "#5b7cfa", "#38e1d6"],
  amplitude = 1.0,
  blend = 0.5,
  speed = 0.5,
  className = "",
  ...
  useEffect(() => {
    ...
    try {
      ...
      return; // no WebGL → fallback gradient stays
    }
    ...
    const update = (t: number) => {
      ...
      const { speed: sp, amplitude: amp, blend: bl, colorStops: cs } = cfg.current;
      program.uniforms.uTime.value = t * 0.001 * sp;
